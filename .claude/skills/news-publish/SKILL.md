...
`prompt.md` の指示に従った結果、今回は発行する内容が無い（保存件数が閾値に満たない等の発行見送り条件に該当する）と判断された場合は、以降のステップ（テンプレート流し込み・ファイル書き出し・ハブ更新・commit・push）を実行せず、見送る理由を報告して終える。ファイルは一切書き換えない。
...
`$ARGUMENTS/prompt.md` に `## 発行後の処理` セクションがあれば、`git push` が成功した後にその指示を実行する。push が失敗した場合は実行しない。このセクションが無いトピックでは何もしない。

外部状態を変える後処理（保存解除など）を push 成功後に限定するための仕組み。途中で失敗したときに、発行できていない記事が外部側からも失われるのを防ぐ。
